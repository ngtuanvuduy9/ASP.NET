...
namespace nguyentuanvuduy_2123110226.Services
{
    public class ProductService : IProductService
    {
        ...
        public async Task<(int Total, IEnumerable<ProductReadDto> Data)> GetAllAsync(int page, int size, int? categoryId, string? keyword = null, int? customerId = null)
        {
        ...
        // ✅ ĐÃ SỬA HÀM NÀY: Truyền thêm trạng thái IsFavorite cho trang chi tiết
        public async Task<ProductReadDto?> GetByIdAsync(int id, int? customerId = null)
        {
            // Kiểm tra xem user này đã thả tim món này chưa
            bool isFavorite = false;
            if (customerId.HasValue)
            {
                isFavorite = await _context.Favorites.AnyAsync(f => f.CustomerId == customerId.Value && f.ProductId == id);
            }
        ...
        public async Task<(bool IsSuccess, int StatusCode, string Message, ProductReadDto? Data)> CreateAsync(ProductCreateDto dto)
        {
            ...
            if (!categoryExists) return (false, 404, $"Không tìm thấy category với Id = {dto.CategoryId}", null);
            ...
            var readDto = new ProductReadDto(
                ...
                product.CreatedAt, product.UpdatedAt, product.CategoryId, categoryName, false
            );
        ...
        public async Task<(bool IsSuccess, int StatusCode, string Message, int Added, int Skipped)> BulkCreateAsync(List<ProductCreateDto> dtos)
        {
            ...
            if (!toAdd.Any()) return (false, 409, "Tất cả mã đều đã tồn tại", 0, dtos.Count);
        ...
        public async Task<(bool IsSuccess, int StatusCode, string Message)> UpdateAsync(int id, ProductUpdateDto dto)
        {
            ...
            if (product == null) return (false, 404, $"Không tìm thấy sản phẩm với Id = {id}");

            bool categoryExists = await _context.Categories.AnyAsync(c => c.Id == dto.CategoryId && c.IsActive);
            if (!categoryExists) return (false, 404, $"Không tìm thấy category với Id = {dto.CategoryId}");
        ...
        {
            if (file == null || file.Length == 0) return (false, "Không có file nào được chọn.", null);

            var allowedExtensions = new[] { ".jpg", ".jpeg", ".png", ".gif" };
            var extension = Path.GetExtension(file.FileName).ToLower();
            if (!allowedExtensions.Contains(extension)) return (false, "Chỉ cho phép tải lên file ảnh (.jpg, .jpeg, .png, .gif)", null);

            if (file.Length > 5 * 1024 * 1024) return (false, "Kích thước ảnh không được vượt quá 5MB", null);
